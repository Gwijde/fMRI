...
config_log( ['logs\fMRI_' subjName '_Sound_IMG_1L.log'] );
...
responseKeys = [keys.Left keys.Right 7 8 34 35];                   % Define response keys ONLY
...
% Visual Task
frame = strcat(stimFolder,'framerounded.jpg');
welcome = strcat(stimFolder,'welcomerounded.jpg');
fixcross = strcat(stimFolder,'fixcrossrounded.jpg');
thanks = strcat(stimFolder,'thanksrounded.jpg');
breakimg = strcat(stimFolder,'breakrounded.jpg');
extension = '.wav';  
...
for a = 1:32
   ...
   % Takes the trial number and derives jitter
    ...
    info{trial,2} = 'Imagery_1L';                                      % If taskCategory is audio, records roundLeft (1 = rounded left, 2 = stretched left)
...
%% The grand finale
    ...
    %%
    save(strcat(subjFolder,'/info','_',listList,'Sounds_IMG_1L','.mat'),('info'));         % This saves the created cell array named 'info' into each participants individually created folder.
    xlswrite(strcat(subjFolder,'_',listList,'Sound_IMG_1L'),info);   
    %%
