...
TESTING = 0;                                                                   % Set to 1 if testing, set to 0 if real fMRI.
...
elseif TESTING == 1
    config_keyboard(100, 1, 'nonexclusive');                                   % Keyboard configurations - 100 = max. number of recorded key reads, 1 = timing resolution in mseconds, nonexclusive = allows keyboard control in other programs
    config_display(0, 5, [0,0,0], [1,1,1], 'Helvetica', 32, 4, 0, 4);          % Cogent window configuration - 1 = full screen, 3 = 1024x768, [0.5,0.5,0.5] = background colour, Helvetica, 32 = font type, font size
    config_sound(1, 16, 22050, 100);     % Cogent sound configurations - 1 = mono, 16 = number of bits, 22050 = number of samples per second, 100 = number of possible buffers)
end
...
config_log( ['logs\fMRI_' subjName '_Sound_IMG_1R.log'] );
...
% Visual Task
frame = strcat(stimFolder,'framespread.jpg');
welcome = strcat(stimFolder,'welcomespread.jpg');
fixcross = strcat(stimFolder,'fixcrossspread.jpg');
thanks = strcat(stimFolder,'thanksspread.jpg');
breakimg = strcat(stimFolder,'breakspread.jpg');
extension = '.wav';  
...
for a = 1:32
    ...
    stimWav = strcat(stimFolder,imageNum,extension);                       
   ...
   % Takes the trial number and derives jitter
    ...
    info{trial,1} = 'Sound';                                         % Records the taskCategory (0 = audio, 1 = hand, 2 = face)
    info{trial,2} = 'Imagery_1R';                                      % If taskCategory is audio, records roundLeft (1 = rounded left, 2 = stretched left)
    ...
    if strcmp(stimCorrect,k) == 1                                    % Assigns value of 1 to variable acc(uracy) if stimCorrect matches key pressed     
        acc = 0;
    elseif strcmp(stimCorrect,k) == 0                                % Assigns value of 0 to variable acc(uracy) if stimCorrect does not match key pressed - SWITCHED because of Excel listing
        acc = 1;
...
%% The grand finale
    ...
    %%
    save(strcat(subjFolder,'/info','_',listList,'Sounds_IMG_1R','.mat'),('info'));         % This saves the created cell array named 'info' into each participants individually created folder.
    xlswrite(strcat(subjFolder,'_',listList,'Sound_IMG_1R'),info);   
    %%
