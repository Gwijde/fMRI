...
config_log( ['logs\fMRI_' subjName '_Hand_RT.log'] );
...
welcome = strcat(stimFolder,'welcomehandRT.jpeg');
...
wait(400);
drawpict(2);                                                           % Display welcome + frame forever                                                    % getkeymap command outputs a list of codes assigned to each key by cogent
...
%% The grand finale
    ...
    %%
    save(strcat(subjFolder,'/info','_',listList,'Hand_RT','.mat'),('info'));         % This saves the created cell array named 'info' into each participants individually created folder.
    xlswrite(strcat(subjFolder,'_',listList,'Hand_RT'),info);   
    %%
